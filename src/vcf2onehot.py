...
def sample_seq_init(samples):
    ref_seq = reference_seq() # lay seq tham chieu; length = 7418
    sample_dict = {}
...
def get_embedding(embeddings, key):
    if key in embeddings:
        return embeddings[key]
    else:
        return None

def build_seqs(vcf, debug=False):
    # Build a dictionary of seqs for each sample using the reference
    samples = get_vcf_subject_ids(vcf) # tên các biến thể
    sample_seqs = sample_seq_init(samples) #  tao dict voi: key=bien the; value=[ref_seq_1, ref_seq_2]       
    ...
    with open(vcf) as f:
        for line in f:
            ...
            # return
            ref_embedding = get_embedding(embeddings, ref_key)
            alt_embedding = [get_embedding(embeddings, k) for k in alt_key]
...
def indices2embeddings(data):
    embeddings = np.apply_along_axis(embedding_mapper, axis=1, arr=data)
    return(embeddings)

def embedding_mapper(x):
    embeddings = create_embedding_matrix()
    return embeddings[x]

def format_seqs(seq_data): # đầu vào là output của build_seqs(vcf)
    sample_names = []
    seqs = []
...
def main():
    ...
    for i in range(n_batch):
        ...
        if i < n_batch_train:
            joblib.dump(data, f'../data/train/{file_name}.joblib')
            print(f"save train batch {i + 1}...")
        else:
            joblib.dump(data, f'../data/test/{file_name}.joblib')
            print(f"save test batch {i + 1}...")
